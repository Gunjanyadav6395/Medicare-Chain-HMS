...
export const createAppointment = async (req, res) => {
  try {
    ...
    const finalServiceImageUrl = svcImageUrlFromDB?.length
      ? svcImageUrlFromDB
      : (serviceImageUrlFromBody && String(serviceImageUrlFromBody).trim()) || "";

    const finalServiceImagePublicId = svcImagePublicIdFromDB?.length
...
export const getAppointmentsById = async (req, res) => {
    try {
        ...
        console.error("getAppointmentsById unexpected error:", err);
        return res.status(500).json({ 
            success: false,
            message: "Server error"
        });
...
// to cancel the serviceApppointment
export const cancelAppointment = async (req, res) => {
    try{
        ...
        const appt = await ServiceAppointment.findById(id);
         ...
         if (appt.payment?.method === "Online" && appt.payment?.status === "Paid") {
...
export const getAppointmentsByPatient = async (req, res) => {
  try {
    const createdBy = resolveClerkUserId(req);

    console.log("FETCH USER:", createdBy);
