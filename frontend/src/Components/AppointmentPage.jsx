...
const AppointmentPage = () => {
  ...
  const loadDoctorAppointments = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        console.log("No Clerk token found for doctor appointments.");
        setDoctorAppts([]);
        return;
      }
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "Clerk token (frontend) for services:",
        token ? `${token.slice(0, 20)}...` : null
      );

      if (!token) {
        console.log("No Clerk token found for service appointments.");
        setServiceAppts([]);
        return;
      }
      ...
      setServiceAppts([]);
      setAppointmentsRaw((p) => ({ ...p, services: [] }));
    } finally {
    ...
    return doctorAppts
      .map((a) => {
       console.log("RAW APPOINTMENT:", a);
        console.log("PATIENT ID ONLY:", a.patientId);
      console.log("PATIENT ID _ID:", a.patientId?._id);
        ...
        <div className={appointmentPageStyles.doctorGrid}>
          {appointmentData.map((item) => (
            <div key={item.id} className={cardStyles.doctorCard}>
              ...
              {item.patientId ? (
                <button
                  onClick={() => navigate(`/patient-history/${item.patientId}`)}
