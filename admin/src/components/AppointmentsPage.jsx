...
const AppointmentsPage = () => {
  ...
  //display all the appt or the filter ones
  ...
  {/* Controls – search left | date + speciality + clear right */}
  <div className="mt-4 md:mt-0 flex flex-col md:flex-row md:items-center gap-4 md:gap-6 w-full md:w-auto">

    {/* ─── Search ─── left side / grows */}
    <div className="w-full md:flex-1 md:max-w-xl">
      <div className="flex items-center bg-white rounded-full px-4 py-2.5 shadow-sm border border-emerald-200 focus-within:border-emerald-400 transition-all">
        <Search size={18} className="text-emerald-500" />
        <input
          type="text"
          placeholder="Search doctor, patient, speciality or mobile"
          className="ml-3 flex-1 bg-transparent outline-none text-sm text-emerald-800 placeholder-emerald-400"
          value={query}
    ...
    <div className="flex items-center gap-3 flex-wrap md:flex-nowrap">
      ...
      {/* Clear button – matches first screenshot */}
      <button
        type="button"
  ...
  <>
    ...
    <div className="mt-6 text-center text-lg text-emerald-700">
      No Appointments Found.
    </div>
  </>
) : displayed.length === 0 ? (
  <div className="mt-10 mx-auto max-w-3xl rounded-2xl bg-white/70 shadow-sm border border-emerald-100/60 py-16 text-center">
    <p className="text-lg font-medium text-emerald-700/90">
      No appointments found.
    </p>
  </div>
) : (
